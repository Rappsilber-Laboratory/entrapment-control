...
def find_in_sequence(protein, peptide):
    ...
    if pos<0:
        #print("looking for "+ peptide.replace("I","L") + " in " + str(protein).replace("I","L") +"\n"
        #      +"instead of " + peptide + " in " + str(protein))
...
def process_mango_comet_xlinkprophet(result_file, proteins, td_fasta_dict):
    ...
    df["isDecoy1"] = df["protein1"].apply(lambda x : all([y.startswith("rev_") for y in x.split(",")]))
    df["isDecoy2"] = df["protein2"].apply(lambda x : all([y.startswith("rev_") for y in x.split(",")]))
    ...
    df["PepPos1"] = df[["basepeptide1","protein1"]].apply(
        lambda x: ";".join([find_in_sequence(td_fasta_dict[y].seq, x[0]) for y in x[1].split(";")]), axis=1)
